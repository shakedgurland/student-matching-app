...
//
// This module turns raw closed-choice answers into product-neutral traits on
// a 1–5 scale (1 = low, 3 = neutral, 5 = high). Free-text fields
// (partner_should_know_text, conversation_starter, green_flag, *_other) are
// intentionally NOT interpreted here — they are deferred to a future AI or
// explicit keyword-mapping commit, per product decision.
//
// Pure functions only. No I/O, no side effects, no dependency on supabase.
// Matching scoring is unchanged in this commit — this is a helper layer that
// later commits will consume.
...
// ---------------------------------------------------------------------------
// Internal mapping tables: one per (question, trait) pair where they differ.
// Values 1–5 mean: 1 = strong low signal, 3 = neutral, 5 = strong high signal.
// ---------------------------------------------------------------------------

// elevatorScenario: "stuck in an elevator with a stranger you don't know"
...
// karaokeChance: "you are at karaoke with friends"
const KARAOKE_TO_CONFIDENCE: Record<string, number> = {
  first_on_stage: 5,
  sing_if_convinced: 3,
  // shots_first: shyness with coping — capped at 2 because confidence is "social without external aid"
  shots_first: 2,
  prefer_watching: 2,
};

// familiarFace: "you see someone you know from class on the street"
...
// feel_interest_response: "when you feel someone is interested in you"
...
// attraction_initiative_style: "when you like someone, you usually..."
...
// spontaneity: "friends call you to fly abroad tomorrow morning" (extreme)
...
// spontaneous_plan_response: "friends call you to come bowling tonight" (everyday)
...
const CONFLICT_TO_ENGAGEMENT: Record<string, number> = {
  talk_immediately: 5,
  need_cooldown: 4, // still engages, just after a cooldown
  situational: 3,
  avoidant: 1,
};
...
// intent_type — base for relationship_intent_seriousness
...
// chemistry_vs_longterm — modifier for seriousness, primary for stability_preference
...
// stability_vs_adventure — modifier for seriousness, primary for stability_preference
...
// personal_space_style — primary for independence_need
...
// religion_importance & religious_level_importance — for religious_lifestyle_importance
...
function clamp1to5(n: number): number {
  ...
  // Round to one decimal so callers can see "2.5" leans low rather than always-integer.
...
function deriveSpontaneityLevel(a: Record<string, unknown>): number | null {
  // Everyday spontaneity carries slightly more weight than extreme travel
  // spontaneity (60/40), per the design audit.
...
function deriveWarmthAffection(a: Record<string, unknown>): number | null {
  // Prefer new love_languages array; fall back to legacy love_language single.
  ...
  // Primary signal: love-language breadth. More picked = broader warmth expression.
  let primary: number;
...
function deriveFamilyOrientation(a: Record<string, unknown>): number | null {
  ...
  const religiousLifestyleHint = religion === 'religious' || religion === 'religious_national' || religion === 'haredi';
  ...
  // Weak correlation only — religious lifestyle ≠ family-oriented; small +1 max.
...
/**
 * Derive a stable set of product-neutral 1–5 traits from a user's
 * questionnaire answers. Returns null for any trait that does not have
 * enough signal in the source answers (e.g., the user did not complete the
 * deep questionnaire, or skipped the relevant questions).
 *
 * `profile` is accepted for forward-compatibility but currently unused;
 * every trait derives from `answers` (the JSONB on questionnaire_answers).
 */
...
/**
 * Extract the user's *declared* preferences from their answers. Returns
 * stable shape with safe defaults — never throws. A future scoring commit
 * will read these against the *other* user's derived traits to apply
 * penalties or hard filters.
 *
 * NOTE: Free-text preference fields (partner_should_know_text,
 * conversation_starter, green_flag, and any *_other text fields) are
 * intentionally NOT included here. They will be handled later by an AI or
 * explicit keyword-mapping commit. Including them now would require text
 * interpretation logic that we explicitly deferred.
 */
